...
App::Prong::DeployDB - Deploy Database Schema

=head1 SYNOPSIS

    use App::Prong::DeployDB;

    App::Prong::DeployDB->new_with_options->run();

    # or explicitly
    App::Prong::DeployDB->new(
        dsn => 'dbi:mysql:dbname=...',
        username => '...',
        password => '...',
        source => \@monikers, # if you want to selectively deploy
        drop_table => $bool, # if you want to add a DROP TABLE before CREATE TABLe
        schema_class => 'Prong::Schema', # if you want to use a different schema
    )->run();
